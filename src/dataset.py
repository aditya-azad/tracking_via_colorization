...
class YoutubeDataset(BaseDataset):
            ...
            self.video_paths.extend(
                glob.glob(
                    os.path.join(
                        self.basepath,
                        cat,
                        f"*[{','.join(YoutubeDataset.ALLOWDED_EXTS)}]",
                    )
                )
            )
    ...
    def __getitem__(self, idx):
        ...
        for frames in self._read_frames_cv(video_path):
            ...
            yield frames
        logger.debug(
            f"Total batches yielded {self.count_total_batch} of size {self.batch}"
        )
    ...
    def _read_frames(self, path, image_batch):
        ...
        for packet in video_container.demux():
            if (
                self.batch_per_video
                and (len(frames) / self.batch) == self.batch_per_video
            ):
                logger.debug(
                    f"video batch limit reached: {len(frames)/self.batch}/{self.batch_per_video}"
                )
                break
...
class DavisDataset(BaseDataset):
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            batch % num_samples == 0
        ), f"Batch size ({batch}) must be multiple of num_samples ({num_samples})"
        self.shuffle = shuffle
        self.batch = batch
        self.num_samples = num_samples
        self.max_batchs = max_batchs
    ...
    def _read_images(self, path, batch):
        ...
        images_path = sorted(
            images_path, key=lambda x: int(os.path.basename(x).split(".")[0])
        )
...
class KineticsDataset:
    def __init__(
        ...
        **kwargs,
        ...
        super(KineticsDataset, self).__init__(transforms=transforms)
        self.basepath = basepath
...
def get_data_loader(
    ...
    **kwargs,
    ...
    dataset = dataset_instance(
        ...
        batch_per_video=batch_per_video,
...
if __name__ == "__main__":
    KineticsDataset("dataset/tracking/kinetics")
